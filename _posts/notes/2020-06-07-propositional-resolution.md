...
On the other hand, recall that $$\phi$$ is *(semantically) valid* in $$\Gamma$$, or $$\Gamma \models \phi$$, if $$\phi$$ is true in every model $$M$$ of $$\Gamma$$ (i.e. $$M \models \phi$$; note and distinguish the two different meanings of $$\models$$). This, in turn, is the archetypical example of a "semantic" entailment relation.
